...
public class VariableStore
{
    ...
    {
        (string[] parts, Database db, string varableName) = ExtractInfo(name);
        if (!db.variables.ContainsKey(varableName))
        {
            variable = null;
            return false;
        }

        variable = db.variables[varableName].Get();
        return true;
    }
    ...
    public static bool TrySetValue<T>(string name, T value)
    {
        ...
        db.variables[varableName].Set(value);
        return true;
    }
    ...
    public static bool HashVariable(string name)
    {
        string[] parts = name.Split(DEFAULT_VARIABLE_RELATIONAL_ID);
        Database db = parts.Length > 1 ? GetDatabase(parts[0]) : defaultDatabase;
        string variableName = parts.Length > 1 ? parts[1] : parts[0];
    ...
    {
        if (TryGetValue(name, out object variable))
        {
            if (variable is T typedValue)
            {
                return typedValue;
            }
            else
            {
                Debug.LogError($"Variable '{name}' is not of type {typeof(T)}");
            }
        }
        else
        {
