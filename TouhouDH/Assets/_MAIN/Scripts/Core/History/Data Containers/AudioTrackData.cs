...
namespace History
{
    ...
    public class AudioTrackData
    {
        ...
        public AudioTrackData(AudioChannel channel)
        {
            ...
            loop = track.loop;
            isContinuousTrack = track.isContinuousTrack;
        }
        ...
        public static void Apply(List<AudioTrackData> data)
        {
            ...
            foreach(var channelData in data)
            {
                ...
                {
                    channelData.trackPath = channelData.trackPath.Replace(" - B", "");
                    AudioClip clip = HistoryCache.LoadAudio((channelData.trackPath));
                    if (clip != null)
                    {
                        channel.StopTrack(immediate: true);
                        //Debug.Log(channelData.isContinuousTrack);
                        if (channelData.isContinuousTrack)
                        {
                            Debug.Log(clip.name);
                            channel.PlayContinuousTrack(clip, channelData.loop, channelData.trackVolume, channelData.trackVolume, channelData.trackPitch, channelData.trackPath);
                        }
                        else
                        {
