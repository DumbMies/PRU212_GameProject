...
namespace History
{
    public class HistoryLogManager : MonoBehaviour
    {
        ...
        [SerializeField] private UnityEngine.UI.Slider logScaleSlider;

        private float textScaling => logScaling * 2f;

        public void Open()
        {
            while (VNMenuManager.instance.isOpen == false)
            {
                while (InputPanel.instance.isWaitingOnUserInput == false)
                {
                    if (isOpen)
                    {
                        return;
        ...
        private IEnumerator SetScrollToBottom()
        {
            yield return null;
