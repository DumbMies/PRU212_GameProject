...
public class AudioManager : MonoBehaviour
{
    ...
    public AudioSource PlayVoice(string filePath, AudioMixerGroup mixer = null, float volume = 1, float pitch = 1, bool loop = false)
    {
    ...
    public AudioTrack PlayContinuousTrack(AudioClip initialClip, int channel = 0, bool loop = true, float startingVolume = 0f, float volumeCap = 1f, float pitch = 1f, string filePath = "")
    {
        AudioChannel audioChannel = TryGetChannel(channel, createIfDoesNotExist: true);
        AudioTrack track = audioChannel.PlayContinuousTrack(initialClip, loop, startingVolume, volumeCap, pitch, filePath);
        return track;
    }
    ...
    {
        volume = audioFalloffCurve.Evaluate(volume);
