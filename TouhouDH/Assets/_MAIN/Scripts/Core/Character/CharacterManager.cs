...
namespace CHARACTERS
{
    public class CharacterManager : MonoBehaviour
    {
        ...
        public Character CreateCharacter(string characterName, bool revealAfterCreation = false)
        {
            if (characters.ContainsKey(characterName.ToLower()))
            {
                Debug.LogWarning($"A character called '{characterName}' already exist. Didn't create the character.");
                return null;
            }

            CHARACTER_INFO info = GetCharacterInfo(characterName);

            Character character = CreateCharacrerFromInfo(info);
        ...
        private Character CreateCharacrerFromInfo(CHARACTER_INFO info)
        {
