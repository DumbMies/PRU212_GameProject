...
namespace COMMANDS
{
    public class CMD_DatabaseExtension_Audio : CMD_DatabaseExtension
    {
        ...
        new public static void Extend(CommandDatabase database)
        {
            ...
            database.AddCommand("playcontinuoussong", new Action<string[]>(PlayContinuousSong));
        ...
        private static void PlaySFX(string[] data)
        {
            ...
            //Try to get the name ot path to the sound effect
        ...
        private static void PlayContinuousSong(string[] data)
        {
            string filepath;
            int channel;
            ...
            PlayContinuousTrack(filepath, channel, parameters);
        }
        ...
        private static void PlayContinuousTrack(string filepath, int channel, CommandParameters parameters)
        {
            float volumeCap;
            float startVolume;
            float pitch;
            ...
            // Load the initial and looped versions of the track
            AudioClip initialClip = Resources.Load<AudioClip>(filepath);

            if (initialClip == null)
            {
                Debug.Log($"Was not able to load initial or looped track '{filepath}'");
                return;
            }
            AudioManager.instance.PlayContinuousTrack(initialClip, channel, false, startVolume, volumeCap, pitch, filepath);
        }
